...
const { getSubcategories, getItems, getItemFromValue } = require('../data/items');
...
async function handleItemRollSelects({ interaction, collections }) {
  ...
  try {
    ...
    if (interaction.customId.startsWith('itemroll_select_subcategory:')) {
      ...
      console.log('[ItemRoll] Subcategory selected:', category, '>', subcategory);
    ...
    if (interaction.customId.startsWith('itemroll_select_users:')) {
      ...
      console.log('[ItemRoll] Creating roll for', selectedUsers.length, 'users...');
    ...
    console.log('[ItemRoll] No handler matched for customId:', interaction.customId);
