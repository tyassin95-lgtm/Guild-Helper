...
class WebServer {
  ...
  /**
   * Reset all party bonuses (same as /resetbonuses Discord command)
   */
  async handleAdminResetBonuses(req, res) {
    try {
      const { guildId } = req.session;

      // Delete all PvP bonuses for this guild
      ...
      res.json({
        success: true,
        message: `PvP bonus reset complete! Reset ${result.deletedCount} bonus record(s) and weekly event counter.`
      });
