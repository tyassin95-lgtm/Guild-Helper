...
class WebServer {
  ...
  registerRoutes() {
    ...
    // API: Bot posts a new DM to a user's inbox
    this.app.post('/api/inbox/messages', async (req, res) => {
      await this.handlePostInboxMessage(req, res);
    });

    // API: Get inbox messages for current user
    ...
    this.app.get('/api/inbox/unread-count', requireAuth, async (req, res) => {
      await this.handleGetInboxUnreadCount(req, res);
    });

    // API: Mark message as read/unread
    this.app.patch('/api/inbox/messages/:id/read', requireAuth, async (req, res) => {
      await this.handleToggleInboxMessageRead(req, res);
    });

    // API: Delete message (soft delete)
    this.app.delete('/api/inbox/messages/:id', requireAuth, async (req, res) => {
      await this.handleDeleteInboxMessage(req, res);
    });
  ...
  /**
   * Bot posts a new DM to a user's inbox.
   * Authenticated via a shared secret (BOT_API_SECRET) rather than user session.
   */
  async handlePostInboxMessage(req, res) {
    try {
      // Authenticate bot request via shared secret
      const authHeader = req.headers['authorization'];
      const botSecret = process.env.BOT_API_SECRET;
      if (!botSecret || !authHeader || authHeader !== `Bearer ${botSecret}`) {
        return res.status(401).json({ error: 'Unauthorized' });
      }

      const { userId, content, discordMessageId } = req.body;

      if (!userId || !content) {
        return res.status(400).json({ error: 'userId and content are required' });
      }

      // Deduplicate by discordMessageId if provided
      if (discordMessageId) {
        const existing = await this.collections.inboxMessages.findOne({ discordMessageId });
        if (existing) {
          return res.json({ success: true, messageId: existing._id, deduplicated: true });
        }
      }
      ...
      const result = await this.collections.inboxMessages.insertOne(message);

      res.json({ success: true, messageId: result.insertedId });
    } catch (error) {
      console.error('Error posting inbox message:', error);
      res.status(500).json({ error: 'Failed to post inbox message' });
    }
  }

  /**
   * Get inbox messages for current user (paginated)
   */
  async handleGetInboxMessages(req, res) {
    try {
      ...
      const [messages, totalCount] = await Promise.all([
        this.collections.inboxMessages
          .find(filter)
          .sort({ timestamp: -1 })
          .skip(skip)
          .limit(limit)
          .toArray(),
        this.collections.inboxMessages.countDocuments(filter)
      ]);
   ...
   */
  async handleGetInboxUnreadCount(req, res) {
    try {
      const userId = req.session.userId;

      const count = await this.collections.inboxMessages.countDocuments({
        userId,
        isRead: false,
        isDeleted: false
      });
   ...
   */
  async handleToggleInboxMessageRead(req, res) {
    try {
      const userId = req.session.userId;
      ...
      const result = await this.collections.inboxMessages.updateOne(
        { _id: new ObjectId(messageId), userId, isDeleted: false },
        { $set: { isRead } }
      );
   ...
   */
  async handleDeleteInboxMessage(req, res) {
    try {
      const userId = req.session.userId;
      const messageId = req.params.id;
