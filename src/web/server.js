...
class WebServer {
  constructor() {
    ...
    this.adminTokens = new Map(); // Tokens for admin panel
   ...
   */
  generateAdminToken(guildId, userId, expiresIn = 3600000) { // 1 hour default
    const token = crypto.randomBytes(32).toString('hex');
  ...
  /**
   * Validate admin token and return associated data
   */
  ...
  registerRoutes() {
    ...
    // Admin panel page
    this.app.get('/admin/:token', async (req, res) => {
      await this.handleAdminPage(req, res);
    });

    // API: Get admin panel data
    this.app.get('/api/admin/:token/data', async (req, res) => {
      await this.handleGetAdminData(req, res);
    });

    // API: Get members list for admin panel
    this.app.get('/api/admin/:token/members', async (req, res) => {
      await this.handleGetAdminMembers(req, res);
    });

    // API: Get upcoming events for party formation
    this.app.get('/api/admin/:token/events', async (req, res) => {
      await this.handleGetAdminEvents(req, res);
    });

    // API: Generate event party editor link
    this.app.post('/api/admin/:token/event-party-editor', async (req, res) => {
      await this.handleGenerateEventPartyEditor(req, res);
    });

    // API: Generate static party editor link
    this.app.post('/api/admin/:token/static-party-editor', async (req, res) => {
      await this.handleGenerateStaticPartyEditor(req, res);
    });

    // API: Reset user wishlist
    this.app.post('/api/admin/:token/reset-wishlist', async (req, res) => {
      await this.handleAdminResetWishlist(req, res);
    });

    // API: Reset user party info
    this.app.post('/api/admin/:token/reset-party-info', async (req, res) => {
      await this.handleAdminResetPartyInfo(req, res);
    });

    // API: Send party info reminder DMs
    this.app.post('/api/admin/:token/remind-party-info', async (req, res) => {
      await this.handleAdminRemindPartyInfo(req, res);
    });

    // API: Send wishlist reminder DMs
    this.app.post('/api/admin/:token/remind-wishlist', async (req, res) => {
      await this.handleAdminRemindWishlist(req, res);
    });

    // API: Get wishlist data for giving items
    this.app.get('/api/admin/:token/wishlists', async (req, res) => {
      await this.handleGetAdminWishlists(req, res);
    });

    // API: Give item to user
    this.app.post('/api/admin/:token/give-item', async (req, res) => {
      await this.handleAdminGiveItem(req, res);
    });

    // API: Get items database for item rolls
    this.app.get('/api/admin/:token/items-database', async (req, res) => {
      await this.handleGetItemsDatabase(req, res);
    });

    // API: Create item roll
    this.app.post('/api/admin/:token/create-item-roll', async (req, res) => {
      await this.handleAdminCreateItemRoll(req, res);
    });

    // API: Get guild channels
    this.app.get('/api/admin/:token/channels', async (req, res) => {
      await this.handleGetAdminChannels(req, res);
    });
  ...
  async handleAdminPage(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
  ...
  /**
   * Handle get admin data API request
   */
  async handleGetAdminData(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Get player count
      const playerCount = await this.collections.partyPlayers.countDocuments({ guildId });

      // Get wishlist submission count
      const wishlistCount = await this.collections.wishlistSubmissions.countDocuments({
        guildId,
        submittedAt: { $exists: true }
      });

      // Get active party count
      const partyCount = await this.collections.parties.countDocuments({
        guildId,
        isReserve: { $ne: true }
      });

      // Get active item rolls
      const activeRollCount = await this.collections.itemRolls.countDocuments({
        guildId,
        closed: { $ne: true },
        endsAt: { $gt: new Date() }
      });
  ...
  /**
   * Handle get members list for admin panel
   */
  async handleGetAdminMembers(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      for (const [memberId, member] of guild.members.cache) {
        ...
        const hasExcludedRole = excludedRoles.some(roleId => member.roles.cache.has(roleId));
        ...
        members.push({
          id: memberId,
          displayName: member.displayName,
          avatarUrl: member.user.displayAvatarURL({ size: 64, format: 'png' }),
          hasPartyInfo: !!(playerInfo?.weapon1 && playerInfo?.weapon2),
          hasWishlist: !!wishlist?.submittedAt,
          cp: playerInfo?.cp || 0,
          role: playerInfo?.role || 'unknown',
          weapon1: playerInfo?.weapon1 || null,
          weapon2: playerInfo?.weapon2 || null
        });
      }
  ...
  /**
   * Handle get upcoming events for party formation
   */
  async handleGetAdminEvents(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Get upcoming events (not closed, event time in future)
      const events = await this.collections.pvpEvents.find({
        guildId,
        eventTime: { $gte: new Date() },
        closed: { $ne: true }
      }).sort({ eventTime: 1 }).toArray();

      // Check which events have party formations
      const enrichedEvents = await Promise.all(events.map(async (event) => {
        ...
        return {
          ...
          rsvpAttendingCount: event.rsvpAttending?.length || 0,
          rsvpMaybeCount: event.rsvpMaybe?.length || 0,
          hasPartiesFormed: !!eventParty?.approved,
          partiesFormationStarted: !!eventParty
        };
      }));
  ...
  /**
   * Handle generate event party editor link
   */
  async handleGenerateEventPartyEditor(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Check if party formation already exists
      let eventParty = await this.collections.eventParties.findOne({
        eventId: new ObjectId(eventId)
      });

      // If no party formation exists, create one with attending members
      if (!eventParty) {
        const guild = await this.client.guilds.fetch(guildId);

        // Get attending and maybe members
        const attendingIds = event.rsvpAttending || [];
        const maybeIds = event.rsvpMaybe || [];
        const allAttendeeIds = [...attendingIds, ...maybeIds];

        // Get player info for all attendees
        ...
        for (const attendeeId of allAttendeeIds) {
          ...
          const member = await guild.members.fetch(attendeeId).catch(() => null);
          if (member) {
            availableMembers.push({
              userId: attendeeId,
              displayName: member.displayName,
              weapon1: playerInfo?.weapon1 || 'Unknown',
              weapon2: playerInfo?.weapon2 || 'Unknown',
              cp: playerInfo?.cp || 0,
              role: playerInfo?.role || 'dps',
              avatarUrl: member.user.displayAvatarURL({ size: 64, format: 'png' })
            });
          }
        }

        // Create initial formation with empty parties
        eventParty = {
          eventId: new ObjectId(eventId),
          guildId,
          processedParties: [],
          availableMembers,
          summary: {
            totalAttendees: availableMembers.length,
            tanks: availableMembers.filter(m => m.role === 'tank').length,
            healers: availableMembers.filter(m => m.role === 'healer').length,
        ...
        await this.collections.eventParties.insertOne(eventParty);
      }

      // Generate token for party editor
      const token = this.generateToken(eventId, userId, 3600000);
      const baseUrl = process.env.WEB_BASE_URL || `http://localhost:${this.port}`;
      const editorUrl = `${baseUrl}/party-editor/${token}`;

      res.json({
        success: true,
        url: editorUrl,
        message: 'Party editor link generated'
      });

    } catch (error) {
      console.error('Error generating event party editor:', error);
      res.status(500).json({ error: 'Failed to generate party editor link' });
    }
  }

  /**
   * Handle generate static party editor link
   */
  async handleGenerateStaticPartyEditor(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Generate token for static party editor
      const token = this.generateStaticPartyToken(guildId, userId, 3600000);
      const baseUrl = process.env.WEB_BASE_URL || `http://localhost:${this.port}`;
      const editorUrl = `${baseUrl}/static-party-editor/${token}`;

      res.json({
        success: true,
        url: editorUrl,
        message: 'Static party editor link generated'
      });

    } catch (error) {
      console.error('Error generating static party editor:', error);
      res.status(500).json({ error: 'Failed to generate static party editor link' });
    }
  }

  /**
   * Handle admin reset user wishlist
   */
  async handleAdminResetWishlist(req, res) {
    try {
      ...
      // Delete the wishlist submission
      const result = await this.collections.wishlistSubmissions.deleteOne({
        userId,
        guildId
      });
  ...
  /**
   * Handle admin reset user party info
   */
  async handleAdminResetPartyInfo(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Delete player info
      const result = await this.collections.partyPlayers.deleteOne({
        userId,
        guildId
      });
      ...
      try {
        ...
        console.error('Failed to update roster:', rosterErr);
      }

      res.json({
        success: true,
        message: result.deletedCount > 0 ? 'Party info reset successfully' : 'No party info found for this user'
      });

    } catch (error) {
      console.error('Error resetting party info:', error);
      res.status(500).json({ error: 'Failed to reset party info' });
    }
  }

  /**
   * Handle admin send party info reminder DMs
   */
  async handleAdminRemindPartyInfo(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      const embed = new EmbedBuilder()
        .setColor('#8b5cf6')
        .setTitle('Party Info Reminder')
        .setDescription('Please set up your party information so we can assign you to the right party for events!')
        .addFields(
          { name: 'What to set up', value: '- Your two weapons\n- Your Combat Power (CP)\n- Optional: Build link and gear screenshot', inline: false },
          { name: 'How to do it', value: 'Use the `/profile` command to open your profile dashboard and update your information.', inline: false }
        )
        .setFooter({ text: `Reminder from ${guild.name}` })
        .setTimestamp();
      ...
      for (const [memberId, member] of guild.members.cache) {
        ...
        // Check if already has party info
        const playerInfo = await this.collections.partyPlayers.findOne({
          userId: memberId,
          guildId
        });
      ...
      console.error('Error sending party info reminders:', error);
      res.status(500).json({ error: 'Failed to send reminders' });
    }
  }

  /**
   * Handle admin send wishlist reminder DMs
   */
  async handleAdminRemindWishlist(req, res) {
    try {
      ...
      // Check if wishlists are frozen
      const wishlistSettings = await this.collections.wishlistSettings.findOne({ guildId });
      if (wishlistSettings?.frozen) {
        return res.status(400).json({ error: 'Wishlists are currently frozen' });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('Please submit your item wishlist so we know what gear you need!')
        .addFields(
          { name: 'What to select', value: '- 1 Archboss Weapon\n- 1 Archboss Armor\n- 1 T3 Weapon\n- Up to 4 T3 Armors\n- Up to 2 T3 Accessories', inline: false },
          { name: 'How to do it', value: 'Use the `/profile` command to open your profile dashboard and go to the Wishlist tab.', inline: false }
        )
        .setFooter({ text: `Reminder from ${guild.name}` })
        .setTimestamp();
      ...
      for (const [memberId, member] of guild.members.cache) {
        ...
        // Check if already has submitted wishlist
        const wishlist = await this.collections.wishlistSubmissions.findOne({
          userId: memberId,
          guildId,
          submittedAt: { $exists: true }
  ...
  /**
   * Handle get wishlists for giving items
   */
  async handleGetAdminWishlists(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      // Get all submitted wishlists
      const wishlists = await this.collections.wishlistSubmissions.find({
        guildId,
        submittedAt: { $exists: true }
      }).toArray();
      ...
      // Create a map of given items by user
      const givenItemsMap = new Map();
      for (const item of givenItems) {
        const key = `${item.userId}-${item.itemId}`;
        givenItemsMap.set(key, item);
      }

      // Enrich wishlists
      const enrichedWishlists = await Promise.all(wishlists.map(async (wl) => {
        const member = await guild.members.fetch(wl.userId).catch(() => null);

        // Combine all items
        const allItems = [
          ...(wl.archbossWeapon || []).map(id => ({ id, category: 'archbossWeapon' })),
          ...(wl.archbossArmor || []).map(id => ({ id, category: 'archbossArmor' })),
          ...(wl.t3Weapons || []).map(id => ({ id, category: 't3Weapons' })),
          ...(wl.t3Armors || []).map(id => ({ id, category: 't3Armors' })),
          ...(wl.t3Accessories || []).map(id => ({ id, category: 't3Accessories' }))
        ];

        // Mark items as given
        const itemsWithStatus = allItems.map(item => ({
          ...item,
          given: givenItemsMap.has(`${wl.userId}-${item.id}`)
        }));
  ...
  /**
   * Handle admin give item to user
   */
  async handleAdminGiveItem(req, res) {
    try {
      ...
      const existing = await this.collections.wishlistGivenItems.findOne({
        userId,
        guildId,
        itemId
      });
      ...
      if (existing) {
        return res.status(400).json({ error: 'Item already given to this user' });
      }

      // Record the given item
      await this.collections.wishlistGivenItems.insertOne({
        userId,
        guildId,
        itemId,
        givenBy: adminUserId,
  ...
  /**
   * Handle get items database for item rolls
   */
  async handleGetItemsDatabase(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
      ...
      const { ITEMS } = require('../features/itemroll/data/items');

      res.json({ items: ITEMS });

    } catch (error) {
      console.error('Error getting items database:', error);
      res.status(500).json({ error: 'Failed to get items database' });
    }
  }

  /**
   * Handle get guild channels for item rolls
   */
  async handleGetAdminChannels(req, res) {
    try {
      const validation = this.validateAdminToken(req.params.token);
  ...
  async handleAdminCreateItemRoll(req, res) {
    try {
      ...
      const { guildId, userId } = validation.data;
      const { itemName, imageUrl, trait, durationMinutes, eligibleUserIds, channelId } = req.body;

      if (!itemName || !durationMinutes || !channelId) {
        return res.status(400).json({ error: 'Item name, duration, and channel are required' });
      }
      ...
      const embed = new EmbedBuilder()
        .setColor('#fbbf24')
        .setTitle(`Item Roll: ${itemName}`)
        .setDescription(trait ? `**Trait:** ${trait}` : 'No trait specified')
        .addFields(
          { name: 'Ends At', value: `<t:${Math.floor(endsAt.getTime() / 1000)}:F>`, inline: true },
          { name: 'Time Left', value: `<t:${Math.floor(endsAt.getTime() / 1000)}:R>`, inline: true },
          { name: 'Participants', value: '0 rolled, 0 passed', inline: false }
        )
        .setFooter({ text: 'Click Roll to participate or Pass to skip' })
        .setTimestamp();
      ...
      const row = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder()
            .setCustomId(`itemroll_roll:${result.insertedId}`)
            .setLabel('Roll')
            .setStyle(ButtonStyle.Primary)
            .setEmoji('🎲'),
          new ButtonBuilder()
            .setCustomId(`itemroll_pass:${result.insertedId}`)
            .setLabel('Pass')
      ...
      res.json({
        ...
        rollId: result.insertedId.toString()
      });
